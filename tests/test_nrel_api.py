...
def test_convert_response_to_dfs(fake_nrel):
    response = fake_nrel.query_for_data(
        location="amarillo", year=2018, attributes=nrel_service.ATTRIBUTES
    )
    metadata, data = nrel_service.convert_response_to_dfs(response)
    assert metadata is not None
    assert data is not None
    assert {col.replace(" ", "_") for col in data.columns} == nrel_service.ATTRIBUTES
